        ...
        if CurrentMenu() then
            ...
            if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
                ...
                if Enabled == true or Enabled == nil then
                    ...
                    RenderSprite(RageUI.GetBadgeDictionary(RageUI.BadgeStyle.Lock, Selected), RageUI.GetBadgeTexture(RageUI.BadgeStyle.Lock, Selected), CurrentMenu.X + SettingsButton.RightBadge.X + CurrentMenu.WidthOffset, CurrentMenu.Y + SettingsButton.RightBadge.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, SettingsButton.RightBadge.Width, SettingsButton.RightBadge.Height, 0, RageUI.GetBadgeColour(RageUI.BadgeStyle.Lock, Selected))
                ...
                if (Enabled) then
                    if Selected and (CurrentMenu.Controls.Select.Active or (Hovered and CurrentMenu.Controls.Click.Active)) then
                        ...
                        if (Callback.onSelected ~= nil) and (Selected) then
                            ret = Callback.onSelected();
                        end
                        if ret == "notNextMenu" then
                            return
                        end
                        if Submenu ~= nil then
                            if Submenu() then
